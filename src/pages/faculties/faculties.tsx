...
import DeleteFacultiesModal from "./deletefaculties";
const FacultiesListPage: FC = function () {
  const [facultys, setfacultys] = useState([]);
  const [searchTerm, setSearchTerm] = useState('');
  const [currentPage, setCurrentPage] = useState(1);
  const [facultyPerPage, setfacultyPerPage] = useState(4);
  ...
  const filteredLocations = facultys.filter((faculty) =>
  ...
  const currentfacultys = filteredLocations.slice(
    indexOfFirstfaculty,
    indexOfLastfaculty
  );
        ...
        <div className="mb-1 w-full">
          <div className="mb-4">
            <Breadcrumb className="mb-4">
              ...
              <Breadcrumb.Item href="/location/list">Khoa ban</Breadcrumb.Item>
          ...
          <div className="sm:flex">
            <div className="mb-3 hidden items-center dark:divide-gray-700 sm:mb-0 sm:flex sm:divide-x sm:divide-gray-100">
              <form className="lg:pr-3">
                <Label htmlFor="facultys-search" className="sr-only">
                  Search
                </Label>
                <div className="relative mt-1 lg:w-64 xl:w-96">
                  <TextInput
                    id="facultys-search"
                    name="facultys-search"
                    placeholder="Tìm kiếm khoa ban"
  ...
  return (
    <Table className="min-w-full divide-y divide-gray-200 dark:divide-gray-600">
      <Table.Head className="bg-gray-100 dark:bg-gray-700 align-center">
        ...
        <Table.HeadCell className="text-center">Khoa Ban</Table.HeadCell>
          ...
          <Table.Row key={item.id} className="hover:bg-gray-100 dark:hover:bg-gray-700">
            ...
            <Table.Cell>
              <div className="flex items-center gap-x-3 whitespace-nowrap">
                <button onClick={() => navigate(`/faculties/list/edit/${item.id}`)}>
                  <EditFacultiesModal />
                </button>


                <Link to={`/faculties/list/delete/${item.id}`}>
                  <DeleteFacultiesModal />
                </Link>
              </div>
            </Table.Cell>
          </Table.Row>
